...
# Read the file
df = pd.read_csv("results/handshake_summary.csv")
...
lab_values   = lab.set_index("scenario").reindex(all_scenarios)["overall_mean_handshake_ms"]
nginx_values = nginx.set_index("scenario").reindex(all_scenarios)["overall_mean_handshake_ms"]
...
ax.set_ylabel("Mean Handshake Time (ms)", fontsize=12)
ax.set_title("Average Handshake Time by Environment and Scenario", fontsize=13, fontweight="bold")
ax.set_xticks(x)
...
plt.tight_layout()
plt.savefig("handshake_time.png", dpi=300)
plt.show()