...
# 1. Definim els perfils i llegim les dades evolutives (WAN)
perfils = [
    ("local", "Local\n(Ideal)"),
...
for nom_arxiu, etiqueta in perfils:
    ruta = f"results/handshake_summary_all_{nom_arxiu}.csv"
    if not os.path.exists(ruta):
        print(f"[ERROR] No s'ha trobat {ruta}.")
        exit(1)
    df = pd.read_csv(ruta)
...
# 2. Llegim les dades estructurals de mida (Utilitzem el local per ser la mateixa a tot arreu)
df_siz = pd.read_csv("results/handshake_message_sizes_all_local.csv")
...
ax2.axhline(y=1500, color="#31AE44", linestyle='-.', label='Límit MTU (~1500 B)', alpha=0.8, zorder=4)
...
plt.tight_layout()
fig2.savefig("results/plot_main_nginx_directional_full.png", dpi=300)

# ==========================================================
# FUNCIÓ PELS GRÀFICS EVOLUTIUS (WAN)
# ==========================================================
x_wan = np.arange(len(etiquetes_x_wan))
width = 0.25 # Fem les barres més estretes per donar cabuda a 3 columnes

def crear_grafic_wan(dades_classic, dades_hybrid, dades_pq, titol, ylabel, arxiu, format_etiqueta, leg_loc='best'):
    fig, ax = plt.subplots(figsize=(10, 6)) # Un pèl més ample per acomodar 3 barres
    
    b_cl = ax.bar(x_wan - width, dades_classic, width, label="Clàssic", color=color_classic, zorder=3)
    b_hy = ax.bar(x_wan, dades_hybrid, width, label="Híbrid", color=color_hybrid, zorder=3)
    b_pq = ax.bar(x_wan + width, dades_pq, width, label="PQ Pur", color=color_pq, zorder=3)
    ...
    ax.set_xlabel("Distància simulada (Perfil de xarxa)", fontsize=11, fontweight="bold", labelpad=10)
    ax.set_ylabel(ylabel, fontsize=11, fontweight="bold")
    ax.set_title(titol, fontsize=14, fontweight="bold", pad=15)
    ax.set_xticks(x_wan)
    ...
    max_val = max(max(dades_classic), max(dades_hybrid), max(dades_pq))
...
crear_grafic_wan(mean_classic, mean_hybrid, mean_pq, "Temps de handshake (Mitjana) - Entorn NGINX", 
                 "Temps (ms)", "plot_mean_nginx_time_full.png", "%.0f ms", leg_loc="upper left")

# GRÀFIC 3: Temps (Mediana)
crear_grafic_wan(median_classic, median_hybrid, median_pq, "Temps de handshake (Mediana) - Entorn NGINX", 
                 "Temps (ms)", "plot_median_nginx_time_full.png", "%.0f ms", leg_loc="upper left")

# GRÀFIC 4: Throughput
crear_grafic_wan(tp_classic, tp_hybrid, tp_pq, "Capacitat del servidor (Throughput) - Entorn NGINX", 
                 "Connexions/segon", "plot_main_nginx_throughput_full.png", "%.1f", leg_loc="upper right")

# GRÀFIC 5: Tail Latency P99
crear_grafic_wan(p99_classic, p99_hybrid, p99_pq, "Tail Latency P99 - Entorn NGINX", 
                 "Temps P99 (ms)", "plot_main_nginx_taillatency_full.png", "%.0f ms", leg_loc="upper left")



print("Generats tots els gràfics principals complets (NGINX FULL) amb PQ pur a /results!")